...
public class ReportServiceImpl implements ReportService {
    ...
    public ReportDTO.OverviewResponse getOverview() {
        ...
        double applyToInterview = totalApps > 0
                ? Math.round(interviewed * 10000.0 / totalApps) / 100.0 : 0;
        double interviewToHired = interviewed > 0
                ? Math.round(hired * 10000.0 / interviewed) / 100.0 : 0;
    ...
    public List<ReportDTO.JobReportResponse> getJobReport() {
        ...
        List<Application> hiredApps = applicationRepository.findByStatus("HIRED");
    ...
    public ReportDTO.CandidateAnalyticsResponse getCandidateAnalytics() {
        ...
        all.forEach(a -> {
            if (a.getJob() != null && a.getJob().getSkills() != null) {
                a.getJob().getSkills().forEach(s ->
                        skillCount.merge(s.getName(), 1L, Long::sum));
            }
        });
        ...
        Map<String, Long> bySource = all.stream()
                ...
                .collect(Collectors.groupingBy(a -> {
                    ...
                    if (email.contains("linkedin")) return "LinkedIn";
                    if (email.contains("gmail"))    return "Website";
                    return "Other";
                }, Collectors.counting()));
    ...
    public ReportDTO.TimeSeriesReportResponse getTimeSeries(String period,
        ...
        List<ReportDTO.TimeSeriesGroup> data = labels.stream().map(label ->
                ReportDTO.TimeSeriesGroup.builder()
                        .label(label)
